...
1. SUM
Es una función de agregación que suma valores numéricos de una columna.
...
FROM orderProduct
```
Esto suma TODAS las cantidades de productos en la tabla.

Otras funciones similares: COUNT(), AVG(), MAX(), MIN()

2. ON
Se usa en los JOIN para especificar cómo se relacionan las tablas (qué columnas conectan una tabla con otra).
Ejemplo:
```
SELECT customer.name, address.city
FROM customer
...
El `ON` dice: "Une las tablas donde el `addressID` del cliente coincida con el `addressID` de la dirección"

**Visualización:**
```
customer.addressID = 5  ←→  address.addressID = 5
```
Solo se unen las filas donde estos valores son iguales.
